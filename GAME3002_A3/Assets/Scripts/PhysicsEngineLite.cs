...
public class PhysicsEngineLite : MonoBehaviour
{
    ...
    //constantly check if there are collisions
        ...
        foreach (PhysicsRB bodyA in rigidBodiesList.GetRange(0, rigidBodiesList.Count - 1))
        {
            foreach (PhysicsRB bodyB in rigidBodiesList.GetRange(rigidBodiesList.IndexOf(bodyA), rigidBodiesList.Count - rigidBodiesList.IndexOf(bodyA)))
            {
                if (bodyA != bodyB)
                {
                    ...
                    // Separating-Axis Theorem test to see if there is a collision
    ...
    //if there is a collision, resolve it such as add a bounce if there is bounciness
    ...
    //slightly correct positions after colliding
    ...
    void FixedUpdate()
    {
        foreach (PhysicsRB rb in rigidBodiesList)
        {
            rb.Integrate(Time.fixedDeltaTime);
        }
        CheckCollisions();
        ResolveCollisions();
    }
